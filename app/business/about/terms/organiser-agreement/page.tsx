...
export default function OrganiserAgreementPage() {
  const [content, setContent] = useState('')
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/legal-docs/organiser-agreement.md')
      ...
      .catch(error => {
        console.error('Error loading organiser agreement:', error)
        setLoading(false)
      })
  }, [])
      ...
      <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
        {loading ? (
          <div className="text-center py-12">
            <p className="text-gray-500">Loading Event Organiser Agreement...</p>
          </div>
        ) : (
          <div className="prose prose-lg max-w-none">
            <ReactMarkdown
              components={{
                ...
                td: ({node, ...props}) => (
                  <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900" {...props} />
                ),
              }}
            >
              {content}
