...
export default function LimitedAgentTermsPage() {
  ...
  useEffect(() => {
    // Redirect to organiser agreement as this page is deprecated
    fetch('/legal-docs/agent-services-addendum.md')
      ...
      .catch(error => {
        console.error('Error loading agent services addendum:', error)
        setLoading(false)
      })
  }, [])
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
        <h1 className="text-4xl font-bold text-masonic-navy mb-8">Agent Services Addendum</h1>
        
        <div className="bg-amber-50 border-l-4 border-amber-400 p-4 mb-8">
          <p className="text-sm">
            <strong>Note:</strong> This document supplements the Event Organiser Agreement for those using LodgeTix payment agent services. Please ensure you have reviewed the <Link href="/business/about/terms/organiser-agreement" className="text-blue-600 hover:text-blue-800">Event Organiser Agreement</Link> first.
          </p>
        </div>

        {loading ? (
          <div className="text-center py-12">
            <p className="text-gray-500">Loading Agent Services Addendum...</p>
          </div>
        ) : (
