-- Fix the upsert_lodge_registration RPC function to use INSERT only (no ON CONFLICT)
-- Only update records when registration_id is provided (for retries)
...
-- Add comment
COMMENT ON FUNCTION public.upsert_lodge_registration IS 'Handles lodge registration creation and updates. Creates new contact records for each registration unless registration_id is provided for retries.';