-- Hotfix for raw_id references in the upsert_individual_registration function
-- This is a simplified version that only updates the error handling section

-- First, let's check which version is currently deployed
DO $$
DECLARE
  v_function_exists boolean;
BEGIN
  -- Check if the function exists with the jsonb parameter signature
  SELECT EXISTS (
    SELECT 1 FROM pg_proc p
    JOIN pg_namespace n ON p.pronamespace = n.oid
    WHERE n.nspname = 'public' 
    AND p.proname = 'upsert_individual_registration'
    AND pg_get_function_arguments(p.oid) = 'p_registration_data jsonb'
  ) INTO v_function_exists;

  IF v_function_exists THEN
    -- This is the version from migration 20250608000026
    -- We need to update it to fix the raw_id references
    ...
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM information_schema.tables 
            ...
            INSERT INTO raw_registrations (
                raw_id,  -- FIXED: was 'registration_id' in the INSERT
                registration_type,
                raw_data,
                processed
            ) VALUES (
                gen_random_uuid(),  -- Generate new raw_id
                'individuals',
                p_registration_data,
                false
            ) RETURNING raw_id INTO v_raw_data_id;  -- Store the raw_id
        END IF;
        ...
        WHEN OTHERS THEN
            ...
            IF v_raw_data_id IS NOT NULL THEN
                ...
                SET error_message = SQLERRM, 
                    processed = false 
                WHERE raw_id = v_raw_data_id;  -- FIXED: was 'id'
            END IF;
    ...
    RAISE NOTICE 'Successfully updated upsert_individual_registration function to fix raw_id references';
  ELSE
    RAISE NOTICE 'Function upsert_individual_registration with jsonb parameter not found - skipping update';
  END IF;
END $$;