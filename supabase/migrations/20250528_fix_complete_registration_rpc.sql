-- Complete fix for registration RPC function with all correct column names
-- This handles masonic_profiles with camelCase columns and attendees with snake_case
...
BEGIN
  ...
  -- Get event title
  SELECT name INTO v_event_title
  FROM events
  WHERE id = (registration_data->>'event_id')::UUID;
  ...
  FOR v_attendee_record IN SELECT * FROM jsonb_array_elements(attendees_data) LOOP
    ...
    INSERT INTO attendees (
      ...
      COALESCE(v_attendee_record.value->>'event_title', v_event_title),
    ...
    -- Create masonic profile if applicable (using camelCase columns)
    ...
    IF v_attendee_record.value->'masonic_profile' IS NOT NULL AND 
      ...
      -- Check if masonic_profiles uses camelCase or snake_case
      BEGIN
        INSERT INTO masonic_profiles (
          attendeeid, -- Using camelCase based on error
          rank,
          masonic_title,
          grand_rank,
          grand_officer,
          grand_office,
          notes,
          createdat, -- Using camelCase
          updatedat  -- Using camelCase
        ) VALUES (
          v_attendee_id,
...
-- Also fix the payment status trigger
...
BEGIN
  ...
  IF OLD.payment_status IS DISTINCT FROM NEW.payment_status THEN
    -- Update tickets only (attendees don't have payment_status)
    UPDATE tickets
    SET 
      ticket_status = CASE 
