-- Fix customer_type enum by handling dependent views
-- The enum should have values: booking_contact, sponsor, donor

-- Step 1: Drop dependent views
DROP VIEW IF EXISTS auth_user_customer_view CASCADE;

-- Step 2: Store any other dependent views/functions
-- (We'll check if there are others and handle them)

-- Step 3: Create a new enum with correct values
DO $$ 
BEGIN
    -- Check if we need to create a new type or if it already has correct values
    IF EXISTS (
        SELECT 1 
        FROM pg_type t 
        JOIN pg_enum e ON t.oid = e.enumtypid 
        WHERE t.typname = 'customer_type' 
        AND e.enumlabel IN ('booking_contact', 'sponsor', 'donor')
    ) THEN
        RAISE NOTICE 'customer_type enum already has correct values';
    ELSE
        -- Rename old type
        ALTER TYPE customer_type RENAME TO customer_type_old;
        
        -- Create new type with correct values
        CREATE TYPE customer_type AS ENUM ('booking_contact', 'sponsor', 'donor');
        ...
        USING 
            CASE 
                WHEN customer_type::text = 'individual' THEN 'booking_contact'::customer_type
                WHEN customer_type::text = 'organisation' THEN 'booking_contact'::customer_type
                WHEN customer_type::text = 'booking_contact' THEN 'booking_contact'::customer_type
                ELSE 'booking_contact'::customer_type
            END;
...
-- Step 4: Recreate the auth_user_customer_view
CREATE OR REPLACE VIEW auth_user_customer_view AS
SELECT 
    ...
    c.customer_type
FROM auth.users au
LEFT JOIN customers c ON c.customer_id = au.id;

-- Step 5: Update the RPC function to use 'booking_contact'
...
BEGIN
    ...
    IF v_contact_id IS NULL THEN
        INSERT INTO contacts (
            ...
            'organisation'::contact_type,  -- contacts table uses 'organisation' for lodges
            v_auth_user_id,
    ...
    INSERT INTO customers (
        ...
        p_booking_contact->>'firstName',
        p_booking_contact->>'lastName',
        'booking_contact'::customer_type  -- Use booking_contact for lodge registrations
    )
...
-- Add comment
COMMENT ON FUNCTION public.upsert_lodge_registration IS 'Handles lodge registration creation and updates. Uses booking_contact as customer_type.';

-- Grant permissions on the view
GRANT SELECT ON auth_user_customer_view TO authenticated;

-- Verify the enum values
DO $$
BEGIN
    RAISE NOTICE 'Customer type enum values are now: booking_contact, sponsor, donor';
    RAISE NOTICE 'auth_user_customer_view has been recreated';
END $$;