-- Fix customer_type enum to have the correct values
-- and update the RPC function to use 'booking_contact'

-- First, we need to update the enum type
-- This requires careful handling as we can't just drop and recreate if it's in use
...
-- Now update the RPC function to use 'booking_contact' (which should work now)
...
BEGIN
    ...
    INSERT INTO customers (
        ...
        p_booking_contact->>'firstName',
        p_booking_contact->>'lastName',
        'booking_contact'::customer_type  -- Now this should work with the fixed enum
    )
...
-- Add comment
COMMENT ON FUNCTION public.upsert_lodge_registration IS 'Handles lodge registration creation and updates. Uses booking_contact as customer_type with corrected enum values.';

-- Verify the enum values
DO $$
BEGIN
    RAISE NOTICE 'Customer type enum values have been updated to: booking_contact, sponsor, donor';
END $$;