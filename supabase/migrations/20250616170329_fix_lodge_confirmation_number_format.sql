-- Update confirmation number constraint to allow hyphens
-- Individual registrations use format: IND-123456AB (with hyphen)
-- So the constraint should allow: (IND|LDG|DEL)-[0-9]{6}[A-Z]{2}
...
-- Update the lodge registration function to generate confirmation numbers with hyphens
-- to match the individual registration format
-- Drop all versions of the function by specifying all possible signatures
...
DO $$ 
BEGIN 
  EXECUTE 'DROP FUNCTION IF EXISTS upsert_lodge_registration';
EXCEPTION 
