-- FINAL FIX: Fix attendee relationship mapping issue in individual registration
-- The issue is that related_attendee_id MUST reference the database-generated attendee_id
-- NOT the frontend attendeeId. This migration implements proper relationship handling.
...
DECLARE
    ...
    v_frontend_to_db_attendee_map jsonb := '{}';
...
BEGIN
    ...
    -- Process primary attendee FIRST
    IF p_registration_data->'primaryAttendee' IS NOT NULL THEN
        ...
        -- Store mapping of frontend attendeeId to database attendee_id
        v_frontend_to_db_attendee_map := jsonb_set(
            v_frontend_to_db_attendee_map,
        ...
        -- Insert primary attendee WITHOUT relationships (will be set later)
        INSERT INTO attendees (
            ...
            true,
            NULL, -- Will be set later if partner exists
    ...
    -- Process additional attendees FIRST (create all attendees without relationships)
    IF p_registration_data->'additionalAttendees' IS NOT NULL THEN
        ...
        LOOP
            ...
            -- Insert additional attendee WITHOUT relationships first
            INSERT INTO attendees (
                attendee_id,
                registration_id,
                attendee_type,
    ...
    -- NOW UPDATE RELATIONSHIPS (second pass) - All attendees exist in database
    -- First handle primary attendee relationships
    IF p_registration_data->'primaryAttendee' IS NOT NULL THEN
        v_attendee := p_registration_data->'primaryAttendee';
        v_primary_attendee_id := (v_frontend_to_db_attendee_map->>v_attendee->>'attendeeId')::uuid;
        
        -- Check if primary attendee has a partner
        v_partner_attendee_id := NULL;
        IF v_attendee->>'partner' IS NOT NULL THEN
            v_partner_attendee_id := (v_frontend_to_db_attendee_map->>v_attendee->>'partner')::uuid;
        END IF;
        
        -- Update primary attendee with partner relationship
        IF v_partner_attendee_id IS NOT NULL THEN
            UPDATE attendees 
    ...
    -- Then handle additional attendee relationships
    IF p_registration_data->'additionalAttendees' IS NOT NULL THEN
        FOR v_attendee IN SELECT * FROM jsonb_array_elements(p_registration_data->'additionalAttendees')
        LOOP
            -- Get the database attendee_id for this frontend attendeeId
            v_attendee_id := (v_frontend_to_db_attendee_map->>v_attendee->>'attendeeId')::uuid;
            
            -- Resolve partner relationship using the mapping
            v_partner_attendee_id := NULL;
            IF v_attendee->>'partnerOf' IS NOT NULL THEN
                v_partner_attendee_id := (v_frontend_to_db_attendee_map->>v_attendee->>'partnerOf')::uuid;
            ELSIF v_attendee->>'partner' IS NOT NULL THEN
                v_partner_attendee_id := (v_frontend_to_db_attendee_map->>v_attendee->>'partner')::uuid;
            ELSIF v_attendee->>'guestOfId' IS NOT NULL THEN
                v_partner_attendee_id := (v_frontend_to_db_attendee_map->>v_attendee->>'guestOfId')::uuid;
            END IF;
            
            -- Update the relationship ONLY if we found a valid partner
            IF v_partner_attendee_id IS NOT NULL THEN
                UPDATE attendees 
                SET related_attendee_id = v_partner_attendee_id,
                    updated_at = CURRENT_TIMESTAMP
    ...
    RETURN jsonb_build_object(
        ...
        'confirmationNumber', v_confirmation_number,
        'frontendToDbMapping', v_frontend_to_db_attendee_map
    );
