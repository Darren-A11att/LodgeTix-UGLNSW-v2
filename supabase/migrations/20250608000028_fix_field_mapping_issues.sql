-- Fix field mapping issues identified in the comprehensive audit

-- 1. Update the upsert_individual_registration RPC function to handle field name variations
-- and contact preference enum case conversion

-- Drop the existing function to recreate it with fixes
DROP FUNCTION IF EXISTS upsert_individual_registration;

-- Recreate the function with field mapping fixes
...
BEGIN
  ...
  BEGIN
    ...
    WHERE raw_id = (
      SELECT raw_id FROM raw_registrations 
      WHERE raw_data->>'paymentIntentId' = "paymentIntentId"
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      WHERE id = (
        SELECT id FROM raw_registrations 
        WHERE raw_data->>'paymentIntentId' = "paymentIntentId"
        ORDER BY created_at DESC 
        LIMIT 1
      );
...
-- Add comments to document the field mapping behavior
COMMENT ON FUNCTION upsert_individual_registration IS 'Handles individual registration with flexible field mapping for email/phone fields and automatic contact preference case conversion';