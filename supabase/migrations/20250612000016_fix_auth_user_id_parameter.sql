-- Fix authentication user ID handling by adding explicit parameter
-- This ensures consistent user ID between registration and payment APIs
...
CREATE OR REPLACE FUNCTION public.upsert_individual_registration(
  ...
  p_auth_user_id uuid DEFAULT NULL::uuid  -- NEW: explicit auth user ID parameter
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables 
    ...
    v_enhanced_registration_data := jsonb_build_object(
      ...
      'authUserId', p_auth_user_id  -- Include explicit auth user ID
    );
  ...
  -- Handle authentication: use explicit parameter if provided, otherwise fallback to auth.uid()
  v_auth_user_id := COALESCE(p_auth_user_id, auth.uid());
  v_is_anonymous := (v_auth_user_id IS NULL);

  IF v_is_anonymous THEN
    -- For anonymous users, generate a customer ID and mark as anonymous
    v_customer_id := gen_random_uuid();
    RAISE NOTICE 'Creating anonymous registration with customer_id: %', v_customer_id;
  ELSE
    -- For authenticated users, use their auth ID as customer ID
    v_customer_id := v_auth_user_id;
    RAISE NOTICE 'Creating authenticated registration with customer_id: % (auth_user_id: %)', v_customer_id, v_auth_user_id;
  END IF;
  ...
  -- Create or update customer record (works for both authenticated and anonymous)
  INSERT INTO customers (
    ...
    v_booking_email,
    v_booking_phone,
    v_auth_user_id, -- Will be NULL for anonymous users
    now(),
    now()
  )
  ...
  -- Create registration (supports both authenticated and anonymous users)
  INSERT INTO registrations (
    ...
    p_function_id,
    v_customer_id,
    v_auth_user_id, -- Use explicit auth_user_id parameter
  ...
  ON CONFLICT (registration_id) DO UPDATE SET
    ...
    auth_user_id = EXCLUDED.auth_user_id, -- Update auth_user_id on conflict
  ...
  -- Process tickets using event_ticket_id field name
  FOR v_ticket IN SELECT * FROM jsonb_array_elements(p_selected_tickets)
  LOOP
    -- Extract event_ticket_id from the ticket
    v_event_ticket_id := (v_ticket->>'event_ticket_id')::uuid;
    
    IF v_event_ticket_id IS NULL THEN
      RAISE EXCEPTION 'Invalid ticket: event_ticket_id is required. Ticket data: %', v_ticket::text;
    END IF;
    
    -- Check if this is a package ticket (which needs special handling)
    IF (v_ticket->>'isPackage')::boolean = true THEN
      -- For packages, we need to expand to individual tickets
      -- For now, skip package tickets and log the issue
      RAISE NOTICE 'Package ticket detected but not yet fully supported: %', v_ticket::text;
      CONTINUE;
    END IF;
    
    -- Get event ID from event_tickets table
    ...
    IF v_event_id IS NULL OR v_ticket_type_id IS NULL THEN
      RAISE EXCEPTION 'Ticket not found: No event ticket found with event_ticket_id %. Available tickets: %', 
        v_event_ticket_id::text,
        (SELECT string_agg(event_ticket_id::text || ' (' || name || ')', ', ') FROM event_tickets LIMIT 10);
    END IF;
  ...
  v_result := jsonb_build_object(
    ...
    'registrationId', v_registration_id,
    'customerId', v_customer_id,
    'authUserId', v_auth_user_id,  -- Include auth_user_id in result for debugging
