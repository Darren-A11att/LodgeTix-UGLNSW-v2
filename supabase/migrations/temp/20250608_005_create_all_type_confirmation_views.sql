-- Create comprehensive confirmation views for all registration types
-- These views will be used by the type-specific confirmation pages

-- 1. Create a generic registration confirmation view that works for all types
...
-- 2. Update the existing individuals view to extend the base view
...
SELECT 
    ...
    -- Tickets array with full event details
    COALESCE(
        jsonb_agg(
            DISTINCT jsonb_build_object(
                ...
                'ticketType', et.ticket_name,
                'ticketDescription', et.description,
                -- Event details
                'eventId', et_event.event_id,
                'eventTitle', et_event.title,
                'eventSubtitle', et_event.subtitle,
                'eventDescription', et_event.description,
                'eventType', et_event.type,
                'eventStart', et_event.event_start,
                'eventEnd', et_event.event_end,
                'eventSlug', et_event.slug,
                'eventLocationId', et_event.location_id,
                'eventImageUrl', et_event.image_url,
                'eventRegalia', et_event.regalia,
                'eventRegaliaDescription', et_event.regalia_description,
                'eventDressCode', et_event.dress_code,
                'eventDegreeType', et_event.degree_type,
                'eventMaxAttendees', et_event.max_attendees,
                'eventSoldCount', et_event.sold_count,
                'eventReservedCount', et_event.reserved_count
            )
...
LEFT JOIN events et_event ON et.event_id = et_event.event_id
...
-- 3. Create lodge registration confirmation view
    ...
    COALESCE(
        jsonb_agg(
            DISTINCT jsonb_build_object(
                ...
                'packageDescription', p.description,
                'memberCount', package_counts.member_count
            )
...
-- Subquery to count members per package
LEFT JOIN LATERAL (
    SELECT 
        pt.package_id,
        COUNT(DISTINCT pt.attendee_id) AS member_count
    FROM tickets pt
    WHERE pt.registration_id = b.registration_id
    GROUP BY pt.package_id
) package_counts ON package_counts.package_id = p.package_id
...
-- 4. Create delegation registration confirmation view
...
SELECT 
    ...
    b.registration_data->>'delegationLeader' AS delegation_leader,
    b.registration_data->>'delegationSize' AS delegation_size,
    ...
    COALESCE(
        jsonb_agg(
            DISTINCT jsonb_build_object(
                'attendeeId', a.attendee_id,
                'isPrimary', a.is_primary,
                'isLeader', a.is_primary, -- Primary attendee is the leader
                'attendeeType', a.attendee_type,
                'firstName', a.first_name,
                'lastName', a.last_name,
                'title', a.title,
...
-- 5. Create a unified view that can fetch any registration type by confirmation number
...
GRANT SELECT ON public.registration_confirmation_base_view TO anon, authenticated;
GRANT SELECT ON public.lodge_registration_confirmation_view TO anon, authenticated;
GRANT SELECT ON public.delegation_registration_confirmation_view TO anon, authenticated;
GRANT SELECT ON public.registration_confirmation_unified_view TO anon, authenticated;

-- Add comments
COMMENT ON VIEW public.registration_confirmation_base_view IS 
'Base view for all registration confirmations with common fields';

COMMENT ON VIEW public.lodge_registration_confirmation_view IS 
'Lodge-specific confirmation view with member and package details';

COMMENT ON VIEW public.delegation_registration_confirmation_view IS 
'Delegation-specific confirmation view with delegate details';
