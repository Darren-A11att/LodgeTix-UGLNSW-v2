-- Add location details to the confirmation views by joining locations table

-- 1. Update the base view to include location details
...
SELECT 
    ...
    fl.name AS function_location_name,
    fl.address AS function_location_address,
    fl.city AS function_location_city,
...
-- 2. Update the individuals view to reflect new base view columns
...
SELECT 
    ...
    -- Tickets array with full event details and location
    COALESCE(
        jsonb_agg(
            DISTINCT jsonb_build_object(
                ...
                'eventLocationName', el.name,
                'eventLocationAddress', el.address,
                'eventLocationCity', el.city,
                'eventLocationState', el.state,
                'eventLocationCountry', el.country,
                'eventLocationPostalCode', el.postal_code
            )
...
LEFT JOIN locations el ON et_event.location_id = el.location_id
...
-- 3. Update lodge view to include the new base view columns
...
-- 4. Update delegation view similarly
...
'Base view for all registration confirmations with common fields including location details';