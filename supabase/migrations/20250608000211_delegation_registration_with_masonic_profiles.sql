-- Enhanced delegation registration RPC with masonic profile creation
-- Builds on existing delegation RPC to add masonic_profiles creation
-- Maintains existing JSONB storage while adding normalized masonic_profiles records
...
BEGIN
    ...
    IF v_delegation_name IS NULL THEN
        RAISE EXCEPTION 'Delegation name is required';
    END IF;
    ...
    INSERT INTO customers (
        ...
        v_delegation_name,  -- Store delegation name as business name
        CURRENT_TIMESTAMP,
        CURRENT_TIMESTAMP
    )
    ...
    INSERT INTO registrations (
        ...
        v_customer_id,
        v_function_id,
        v_grand_lodge_id,  -- Store grand lodge as organisation
        v_delegation_name,  -- Store delegation name as organisation name
        'delegation',
    ...
    -- Process delegates (similar to attendees but all share same grand_lodge_id)
            ...
            IF v_attendee_type = 'mason' AND v_attendee->>'rank' IS NOT NULL THEN
                INSERT INTO masonic_profiles (
                    ...
                    CASE 
                        ...
                        ELSE NULL
                    END,
                    v_grand_lodge_id, -- Use delegation grand lodge
                    CURRENT_TIMESTAMP,
                    CURRENT_TIMESTAMP
                )
