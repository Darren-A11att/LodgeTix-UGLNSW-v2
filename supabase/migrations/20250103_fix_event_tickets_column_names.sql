-- Fix the upsert_lodge_registration RPC function to use correct event_tickets column names
-- The event_tickets table uses 'event_ticket_id' as primary key, not 'id'
...
BEGIN
    ...
    IF p_registration_id IS NOT NULL THEN
        ...
        UPDATE registrations SET
            payment_status = p_payment_status::payment_status,  -- Cast to enum
    ...
    ELSE
        ...
        INSERT INTO registrations (
            ...
            'pending',
            p_payment_status::payment_status,  -- Cast to enum
    ...
    IF p_registration_id IS NULL OR p_payment_status = 'completed' THEN
        ...
        LOOP
            ...
            v_ticket_count := p_table_count * COALESCE(v_included_item.qty, 10) * COALESCE((item).quantity, 1);
...
-- Add comment
COMMENT ON FUNCTION public.upsert_lodge_registration IS 'Handles lodge registration creation and updates. Fixed to use correct event_tickets column names (event_ticket_id instead of id).';