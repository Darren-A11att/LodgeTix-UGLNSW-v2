-- Comprehensive migration to fix all column naming mismatches
-- Based on database.types.ts analysis

-- 1. Fix attendees table foreign key reference
-- The foreign key references show attendeeid (all lowercase) not attendeeId
-- This is likely causing the masonic_profiles issue

-- 2. Fix the complete_registration RPC to use correct column names
...
BEGIN
    ...
    BEGIN
        ...
        -- Create attendees with correct column names (all lowercase)
        FOR attendee_record IN SELECT * FROM json_array_elements(attendees_data) LOOP
            INSERT INTO attendees (
                attendeeid,  -- lowercase!
                registrationid,  -- lowercase!
                person_id,
                attendeetype,  -- lowercase!
                relatedattendeeid,  -- lowercase!
                relationship,
                contactpreference,  -- lowercase!
                dietaryrequirements,  -- lowercase!
                specialneeds,  -- lowercase!
                eventtitle  -- lowercase!
            ) VALUES (
        ...
        -- Create masonic profiles if provided with correct column names (all lowercase)
            ...
            FOR masonic_profile_record IN SELECT * FROM json_array_elements(masonic_profiles_data) LOOP
                ...
                IF masonic_profile_record.value->>'attendee_id' IS NOT NULL THEN
                    INSERT INTO masonicprofiles (
                        masonicprofileid,  -- lowercase!
                        person_id,
                        rank,
                        masonictitle,  -- lowercase!
                        grandrank,  -- lowercase!
                        grandofficer,  -- lowercase!
                        grandoffice,  -- lowercase!
                        lodgeid  -- lowercase!
                    ) VALUES (
                        gen_random_uuid(),
...
-- 3. Update the update_payment_status_and_complete function to use correct column names
