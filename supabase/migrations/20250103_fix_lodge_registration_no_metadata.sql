-- Fix the upsert_lodge_registration RPC function to remove metadata column reference
-- The customers table doesn't have a metadata column
...
BEGIN
    ...
    -- Create or get customer record (without metadata column)
...
-- Add comment
COMMENT ON FUNCTION public.upsert_lodge_registration IS 'Handles lodge registration creation and updates. Fixed to work with actual customers table schema (no metadata column).';