-- Fix all enum mismatches in the upsert_individual_registration function
-- This migration fixes:
-- 1. contact_type: 'customer' -> 'individual', 'attendee' -> 'individual'
-- 2. attendee_contact_preference: 'Directly' -> 'directly'
...
BEGIN
    ...
    INSERT INTO contacts (
        ...
        'individual',  -- FIXED: Was 'customer', now using valid contact_type enum value
        COALESCE(p_registration_data->'billingDetails'->>'firstName', ''),
        COALESCE(p_registration_data->'billingDetails'->>'lastName', ''),
    ...
    IF p_registration_data->'primaryAttendee' IS NOT NULL THEN
        ...
        v_contact_preference := LOWER(COALESCE(v_attendee->>'contactPreference', 'directly'));  -- FIXED: Ensure lowercase
        ...
        -- Map 'directly' to 'directly' (lowercase) for attendee_contact_preference enum
        IF v_contact_preference = 'directly' AND (v_attendee_email IS NOT NULL OR v_attendee_phone IS NOT NULL) THEN
            ...
            INSERT INTO contacts (
                ...
                v_contact_id,
                'individual',  -- FIXED: Was 'attendee', now using valid contact_type enum value
                v_attendee->>'firstName',
                v_attendee->>'lastName',
...
-- Also create a simple debug function to check enum values
CREATE OR REPLACE FUNCTION public.debug_enum_values() 
RETURNS TABLE (
    enum_type text,
    enum_values text[]
) AS $$
...
GRANT EXECUTE ON FUNCTION public.debug_enum_values() TO authenticated;
GRANT EXECUTE ON FUNCTION public.debug_enum_values() TO anon;