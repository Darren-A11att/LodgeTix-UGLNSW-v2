-- Fix column references in get_event_with_details function
-- The packages table uses package_price instead of price, and doesn't have status or attendee_limit columns
        ...
        'packages', COALESCE(
            (SELECT json_agg(
                json_build_object(
                    ...
                    'price', p.package_price,  -- Fixed: was p.price
                    ...
                    'includes_description', p.includes_description,
                    'parent_event_id', p.parent_event_id,
                    'event_id', p.event_id
                )
                ORDER BY p.package_price  -- Fixed: was p.price
            )
            FROM packages p
            WHERE (p.parent_event_id = v_event_id OR p.event_id = v_event_id)
              AND p.is_active = true  -- Changed from p.status = 'Active'
            ), '[]'::json
        ),
...
-- Grant execute permission (in case they weren't already granted)
