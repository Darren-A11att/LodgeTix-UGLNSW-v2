-- Fix confirmation number format to match the actual database constraint
-- The constraint expects: ^[A-Z]{2,4}-[0-9]{6}$ or ^[A-Z]{3,10}-[0-9A-Z]{4,10}$
-- We'll use the first pattern: LDG-123456
...
BEGIN
  ...
  -- Format: LDG-123456 (matches constraint ^[A-Z]{2,4}-[0-9]{6}$)
  IF p_payment_status = 'completed' THEN
    v_confirmation_number := 'LDG-' || LPAD((RANDOM() * 999999)::INTEGER::TEXT, 6, '0');
  END IF;
...
-- Also fix the standalone generate_confirmation_number function to match the constraint
...
BEGIN
  ...
  -- Format: (IND|LDG|DEL)-123456 (matches constraint ^[A-Z]{2,4}-[0-9]{6}$)
...
DO $$
BEGIN
  RAISE NOTICE 'Fixed confirmation number format to match constraint: ^[A-Z]{2,4}-[0-9]{6}$';
  RAISE NOTICE 'Now generates format like: LDG-123456, IND-789012, DEL-345678';
  RAISE NOTICE 'This matches the first pattern in the database constraint';
END $$;