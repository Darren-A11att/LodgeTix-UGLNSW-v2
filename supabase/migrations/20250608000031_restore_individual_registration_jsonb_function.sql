-- Restore upsert_individual_registration function with JSONB parameter signature
-- This fixes the function signature mismatch between API and database

-- Drop the individual parameter version (from migration 20250608000029)
DROP FUNCTION IF EXISTS public.upsert_individual_registration(
  "registrationId" uuid, "functionId" uuid, "eventId" uuid, "eventTitle" text,
  "registrationType" text, "primaryAttendee" jsonb, "additionalAttendees" jsonb,
  "tickets" jsonb, "totalAmount" numeric, "subtotal" numeric, "stripeFee" numeric,
  "paymentIntentId" text, "billingDetails" jsonb, "agreeToTerms" boolean,
  "billToPrimaryAttendee" boolean, "authUserId" uuid, "paymentCompleted" boolean
);

-- Create the JSONB version that the API expects
...
BEGIN
    ...
    INSERT INTO registrations (
        ...
        registration_data,
        organisation_id,  -- Add organisation_id
        created_at,
        updated_at
    ) VALUES (
        v_registration_id,
        ...
        v_organisation_id,  -- Set organisation_id from primary attendee
        CURRENT_TIMESTAMP,
        CURRENT_TIMESTAMP
    )
...
COMMENT ON FUNCTION public.upsert_individual_registration(jsonb) IS 'Individual registration RPC function with JSONB parameter signature. Restored to fix API compatibility issue. Based on migration 20250608000027 with fixes from 20250608000030.';