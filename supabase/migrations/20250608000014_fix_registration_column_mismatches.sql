-- Fix all column mismatches in the upsert_individual_registration function
...
-- Create a function to check column existence for debugging
CREATE OR REPLACE FUNCTION public.debug_table_columns() 
RETURNS TABLE (
    table_name text,
    column_name text,
    data_type text,
...
BEGIN
    ...
    AND c.table_name IN ('registrations', 'customers', 'contacts', 'attendees', 'tickets')
    ORDER BY c.table_name, c.ordinal_position;
END;
$$ LANGUAGE plpgsql;

-- Grant access to debug function
GRANT EXECUTE ON FUNCTION public.debug_table_columns() TO authenticated;
GRANT EXECUTE ON FUNCTION public.debug_table_columns() TO anon;