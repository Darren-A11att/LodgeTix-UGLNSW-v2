-- Fix the create_registration function to use the correct column names for masonic_profiles
-- This assumes masonic_profiles uses camelCase columns (attendeeid, createdat, updatedat)
...
BEGIN
  ...
  FOR v_attendee_record IN SELECT * FROM jsonb_array_elements(attendees_data) LOOP
    ...
    -- IMPORTANT: masonic_profiles uses camelCase columns (attendeeid, createdat, updatedat)
...
-- Also ensure the required columns exist on attendees table
