-- Fix ticket lookup error handling in individual registration function
-- Add proper validation and error messages for ticket lookup failures
...
BEGIN
  ...
  -- Process tickets with improved error handling
  FOR v_ticket IN SELECT * FROM jsonb_array_elements(p_selected_tickets)
  LOOP
    -- Extract and validate ticket type ID
    v_ticket_type_id_param := (v_ticket->>'ticketTypeId')::uuid;
    
    IF v_ticket_type_id_param IS NULL THEN
      RAISE EXCEPTION 'Invalid ticket: ticketTypeId is required for ticket %', v_ticket::text;
    END IF;
    
    -- Get event ID from ticket type using correct column name with error handling
    SELECT et.event_id, et.event_ticket_id
    INTO v_event_id, v_ticket_type_id
    FROM event_tickets et
    WHERE et.event_ticket_id = v_ticket_type_id_param;

    -- Check if ticket was found
    IF v_event_id IS NULL OR v_ticket_type_id IS NULL THEN
      RAISE EXCEPTION 'Ticket not found: No event ticket found with ID %. Available tickets: %', 
        v_ticket_type_id_param::text,
        (SELECT string_agg(event_ticket_id::text, ', ') FROM event_tickets LIMIT 10);
    END IF;
