-- Fix ALL enums and column names in the upsert_lodge_registration RPC function
-- Based on the actual database types from shared/types/database.ts
...
-- Add comment with summary of all fixes
COMMENT ON FUNCTION public.upsert_lodge_registration IS 'Handles lodge registration creation and updates. 
Fixed to use correct database types:
- contact_type: organisation (not booking_contact)
- customer_type: booking_contact
- payment_status: properly cast to enum
- registration_type: lodge
- event_tickets: uses event_ticket_id column (not id)
- package_item composite type: has event_ticket_id and quantity fields';

-- Verify the enum values are correct
DO $$
BEGIN
    RAISE NOTICE 'Function updated with correct enum values:';
    RAISE NOTICE '- contact_type: organisation';
    RAISE NOTICE '- customer_type: booking_contact';
    RAISE NOTICE '- registration_type: lodge';
    RAISE NOTICE '- payment_status: cast from text parameter';
    RAISE NOTICE 'And correct column names:';
    RAISE NOTICE '- event_tickets.event_ticket_id (not id)';
    RAISE NOTICE '- package_item.event_ticket_id and quantity';
END $$;