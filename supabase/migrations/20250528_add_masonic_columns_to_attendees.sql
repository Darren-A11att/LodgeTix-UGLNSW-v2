-- Add masonic-specific columns to attendees table instead of using separate masonic_profiles table
-- This is a simpler approach that keeps all attendee data in one place

ALTER TABLE attendees
ADD COLUMN IF NOT EXISTS rank TEXT,
ADD COLUMN IF NOT EXISTS masonic_title TEXT,
...
ADD COLUMN IF NOT EXISTS grand_officer_status TEXT,
ADD COLUMN IF NOT EXISTS present_grand_officer_role TEXT,
ADD COLUMN IF NOT EXISTS past_titles TEXT,
ADD COLUMN IF NOT EXISTS notes TEXT;

-- Now update the RPC function to store masonic data in attendees table
...
BEGIN
  ...
  FOR v_attendee_record IN SELECT * FROM jsonb_array_elements(attendees_data) LOOP
    ...
    -- Create attendee with snake_case column names and masonic data
    v_attendee_id := (v_attendee_record.value->>'attendee_id')::UUID;
    
    -- Check if we have masonic data
    v_has_masonic_data := v_attendee_record.value->'masonic_profile' IS NOT NULL AND 
                          v_attendee_record.value->>'attendee_type' = 'mason';
    ...
    INSERT INTO attendees (
      ...
      email,
      phone,
      -- Masonic fields (NULL for non-masons)
      rank,
      masonic_title,
      grand_rank,
      grand_officer,
      ...
      -- Masonic fields
      CASE WHEN v_has_masonic_data THEN v_attendee_record.value->'masonic_profile'->>'rank' ELSE NULL END,
      CASE WHEN v_has_masonic_data THEN v_attendee_record.value->'masonic_profile'->>'masonic_title' ELSE NULL END,
      CASE WHEN v_has_masonic_data THEN v_attendee_record.value->'masonic_profile'->>'grand_rank' ELSE NULL END,
      CASE WHEN v_has_masonic_data THEN v_attendee_record.value->'masonic_profile'->>'grand_officer' ELSE NULL END,
      CASE WHEN v_has_masonic_data THEN v_attendee_record.value->'masonic_profile'->>'grand_office' ELSE NULL END,
      CASE WHEN v_has_masonic_data THEN v_attendee_record.value->'masonic_profile'->>'notes' ELSE NULL END,
      -- Lodge fields
      (v_attendee_record.value->>'grandlodge_org_id')::UUID,
      (v_attendee_record.value->>'lodge_org_id')::UUID,
      v_attendee_record.value->>'lodge_name_number',
      NOW(),
      NOW()
    );
    ...
    v_attendee_results := v_attendee_results || jsonb_build_object(
      ...
      'has_masonic_data', v_has_masonic_data
    );
  END LOOP;
