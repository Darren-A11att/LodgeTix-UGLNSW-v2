-- Add new columns to existing raw_registrations table
ALTER TABLE public.raw_registrations 
...
BEGIN
    IF EXISTS (
        ...
        CREATE INDEX IF NOT EXISTS idx_raw_registrations_registration_id 
        ON public.raw_registrations(registration_id);
    END IF;
END $$;

-- Index on created_at should already exist from remote schema

-- Grant permissions
GRANT SELECT, INSERT ON public.raw_registrations TO authenticated;
GRANT SELECT, INSERT ON public.raw_registrations TO service_role;

-- Update the upsert_individual_registration function to log raw data
...
BEGIN
    ...
    INSERT INTO contacts (
        ...
        COALESCE(p_registration_data->'billingDetails'->>'firstName', ''),
        COALESCE(p_registration_data->'billingDetails'->>'lastName', ''),
        -- Map emailAddress to email field, check primaryAttendee->email first
        COALESCE(
        ...
        -- Map mobileNumber to mobile_number field, check primaryAttendee->mobileNumber first
        COALESCE(
    ...
    IF p_registration_data->'primaryAttendee' IS NOT NULL THEN
        ...
        -- Frontend sends 'email' and 'mobileNumber', not 'primaryEmail' and 'primaryPhone'
        v_attendee_email := COALESCE(
            v_attendee->>'email',
            v_attendee->>'primaryEmail',  -- fallback for backward compatibility
            p_registration_data->'billingDetails'->>'emailAddress',
            p_registration_data->'billingDetails'->>'email'
        );
        ...
        v_attendee_phone := COALESCE(
            ...
            v_attendee->>'phone',
            v_attendee->>'primaryPhone',  -- fallback for backward compatibility
            p_registration_data->'billingDetails'->>'mobileNumber',
            p_registration_data->'billingDetails'->>'phone'
        );
