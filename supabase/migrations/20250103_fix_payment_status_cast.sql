-- Fix the upsert_lodge_registration RPC function to properly cast payment_status
-- The payment_status column uses an enum type, so we need to cast the text parameter
...
-- Add comment
COMMENT ON FUNCTION public.upsert_lodge_registration IS 'Handles lodge registration creation and updates. Fixed to properly cast payment_status to enum type.';