-- Fix column references in upsert_individual_registration function
-- The raw_registrations table uses different column names than expected
...
COMMENT ON FUNCTION public.upsert_individual_registration(jsonb) IS 'Individual registration RPC function with JSONB parameter signature. Fixed column references for raw_registrations table compatibility.';