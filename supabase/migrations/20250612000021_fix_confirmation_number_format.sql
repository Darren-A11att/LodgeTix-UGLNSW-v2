-- Fix confirmation number format to match the database constraint
-- Pattern should be: (IND|LDG|DEL)[0-9]{6}[A-Z]{2} - NO HYPHENS
...
BEGIN
  ...
  -- Format: LDG + 6 digits + 2 uppercase letters (NO HYPHENS)
  -- Example: LDG123456AB
...
-- Also fix the standalone generate_confirmation_number function to match the format
...
BEGIN
  ...
  -- Format: (IND|LDG|DEL) + 6 digits + 2 uppercase letters
...
DO $$
BEGIN
  RAISE NOTICE 'Fixed confirmation number format to match constraint: (IND|LDG|DEL)[0-9]{6}[A-Z]{2}';
  RAISE NOTICE 'Removed hyphens from confirmation numbers';
  RAISE NOTICE 'Examples: LDG123456AB, IND789012CD, DEL345678EF';
END $$;