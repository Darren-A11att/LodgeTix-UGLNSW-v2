-- Fix complete_payment RPC function to use correct payment_status enum value
-- The function was trying to use 'paid' which is not a valid enum value
-- Valid values are: pending, completed, failed, refunded, partially_refunded, cancelled, expired
...
'Completes the payment process for a registration. Updates tickets from reserved to sold, generates confirmation number, updates counts, and returns confirmation details. Fixed to use correct payment_status enum value.';

-- Grant execute permission (maintaining existing permissions)
