-- Fix production error: column "ticket_type" of relation "tickets" does not exist
-- This migration ensures the correct function version is deployed in production

-- Drop any conflicting versions of the function first
...
-- Recreate the correct version that uses event_ticket_id instead of ticket_type
...
BEGIN
  ...
  -- IMPORTANT: This is the fix - use event_ticket_id NOT ticket_type
  IF p_package_id IS NOT NULL THEN
    ...
    IF v_event_id IS NOT NULL AND v_event_ticket_id IS NOT NULL THEN
      INSERT INTO tickets (
        ...
        package_id,
        event_id,
        event_ticket_id,  -- Use event_ticket_id, NOT ticket_type
        ticket_price,
        price_paid,
        status,
        ticket_status,
      ...
      SELECT 
        ...
        p_package_id,
        v_event_id,
        v_event_ticket_id,  -- Use event_ticket_id, NOT ticket_type
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION upsert_lodge_registration(UUID, UUID, INTEGER, JSONB, JSONB, TEXT, TEXT, UUID, DECIMAL, DECIMAL, DECIMAL, DECIMAL, JSONB, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION upsert_lodge_registration(UUID, UUID, INTEGER, JSONB, JSONB, TEXT, TEXT, UUID, DECIMAL, DECIMAL, DECIMAL, DECIMAL, JSONB, TEXT) TO anon;
GRANT EXECUTE ON FUNCTION upsert_lodge_registration(UUID, UUID, INTEGER, JSONB, JSONB, TEXT, TEXT, UUID, DECIMAL, DECIMAL, DECIMAL, DECIMAL, JSONB, TEXT) TO service_role;

-- Log what we did
DO $$
BEGIN
  RAISE NOTICE 'PRODUCTION FIX: Fixed lodge registration function to use event_ticket_id instead of ticket_type';
  RAISE NOTICE 'This resolves the production error: column "ticket_type" of relation "tickets" does not exist';
  RAISE NOTICE 'Updated ticket creation to use event_id and event_ticket_id columns only';
  RAISE NOTICE 'Function now creates tickets with correct table structure matching production schema';
END $$;