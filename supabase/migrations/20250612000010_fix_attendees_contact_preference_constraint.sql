-- Fix attendees table contact_preference constraint issue
-- Update function to provide required contact_preference value
...
BEGIN
  ...
  -- Process attendees with required contact_preference
  ...
  LOOP
    ...
    -- Create attendee record with required contact_preference
    INSERT INTO attendees (
      ...
      COALESCE(v_attendee->>'dietaryRequirements', ''),
      COALESCE(v_attendee->>'additionalInfo', v_attendee->>'specialNeeds', ''),
      COALESCE(v_attendee->>'contactPreference', 'directly')::attendee_contact_preference,
      COALESCE(v_attendee->>'email', ''),
      COALESCE(v_attendee->>'mobile', v_attendee->>'mobileNumber', ''),
      v_attendee,
      now(),
