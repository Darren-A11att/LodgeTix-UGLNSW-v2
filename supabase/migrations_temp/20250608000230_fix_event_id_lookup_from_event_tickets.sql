-- Fix ticket creation to lookup event_id from event_tickets table using eventTicketId
...
DECLARE
    ...
    v_ticket_event_id uuid;  -- For looking up event_id from event_tickets
BEGIN
    ...
    IF p_registration_data->'primaryAttendee' IS NOT NULL THEN
        ...
        -- Add to attendee mapping for tickets (map frontend attendeeId to database attendee_id)
    ...
    -- Process tickets LAST using the correct attendee IDs and lookup event_id from event_tickets
    IF p_registration_data->'tickets' IS NOT NULL THEN
        ...
        LOOP
            -- Get the correct database attendee_id from our mapping
            DECLARE
                v_frontend_attendee_id text := v_ticket->>'attendeeId';
                v_db_attendee_id uuid;
                v_event_ticket_id uuid := (v_ticket->>'eventTicketId')::uuid;
            BEGIN
                v_db_attendee_id := (v_attendee_ticket_map->>v_frontend_attendee_id)::uuid;
                
                -- If we don't have a mapping, use the primary attendee
                IF v_db_attendee_id IS NULL THEN
                    v_db_attendee_id := v_primary_attendee_id;
                END IF;
                
                -- Lookup event_id from event_tickets table using eventTicketId
                SELECT event_id INTO v_ticket_event_id
                FROM event_tickets 
                WHERE event_ticket_id = v_event_ticket_id;
                
                -- If we couldn't find the event_id, use the registration event_id as fallback
                IF v_ticket_event_id IS NULL THEN
                    v_ticket_event_id := (p_registration_data->>'eventId')::uuid;
                END IF;
                
                -- Insert ticket with correct attendee_id and event_id
                INSERT INTO tickets (
                    ticket_id,
                    attendee_id,
                    registration_id,
                    ...
                    v_db_attendee_id,
                    v_registration_id,
                    v_ticket_event_id,  -- Use the looked up event_id
                    v_event_ticket_id,
    ...
    -- Return result with contact_id for billing contact reference
