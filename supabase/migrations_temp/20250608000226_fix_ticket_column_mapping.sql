-- Fix ticket column mapping - use existing columns or add missing ones

-- First, let's see what columns exist in the tickets table and adjust our function accordingly
...
BEGIN
    ...
    -- Process tickets using existing column names (no ticket_number)
    IF p_registration_data->'tickets' IS NOT NULL THEN
        ...
        LOOP
            -- Use the core ticket fields that definitely exist
            INSERT INTO tickets (
                ticket_id,
                attendee_id,
                registration_id,
                ...
                'Active',  -- Use existing status value
                CURRENT_TIMESTAMP,
                CURRENT_TIMESTAMP
            );
        END LOOP;
