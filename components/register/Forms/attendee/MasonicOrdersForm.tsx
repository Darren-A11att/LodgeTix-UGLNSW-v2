...
interface MasonicOrdersFormProps {
  minTables?: number;
  maxTables?: number;
  onComplete?: () => void;
  className?: string;
...
export const MasonicOrdersForm: React.FC<MasonicOrdersFormProps> = ({
  minTables = 1,
  maxTables = 10,
  onComplete,
  className,
  ...
  // Update Grand Lodge for primary attendee - No Lodge selection needed for Masonic Orders
  ...
  const handleComplete = useCallback(() => {
    formSaveManager.saveBeforeNavigation().then(() => {
      if (!selectedGrandLodge) {
        alert('Please select a Masonic Order');
        return;
      }
  ...
  return (
    <div className={cn("space-y-6", className)}>
      {/* Masonic Order Selection with integrated Booking Contact */}
      <div className="relative">
        <Card className="border-2 border-primary/20">
          ...
          <CardContent className="space-y-6 pt-6">
            {/* Masonic Order Selection Field - No Lodge selection needed */}
              ...
              {delegationMembers.length > 0 && (
                <Alert className="mt-6 border-amber-200 bg-amber-50">
                  ...
                  <AlertDescription className="text-amber-700 text-sm">
                    All delegation members will be registered under {selectedGrandLodge ? 'the selected Masonic Order' : 'your Masonic Order'}.
...
// Summary view for Masonic Order orders
export const MasonicOrderFormSummary: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold">
        {isPurchaseOnly ? 'Masonic Order Ticket Purchase' : 'Masonic Order Delegation'} Summary
      </h3>
      ...
      <Card>
        <CardContent className="pt-4">
          <div className="space-y-3">
            <div>
              <p className="font-medium text-lg">{masonicOrderName}</p>
