# Task 046: Create GrandOfficerFields Component

## Objective
Create the GrandOfficerFields component for Mason-specific grand officer details that appears when rank is 'GL'.
...
## Reference Files
- `components/register/oldforms/mason/MasonGrandLodgeFields.tsx`
- Implementation details in CLAUDE.md

## Steps

1. Create `components/register/forms/mason/utils/GrandOfficerFields.tsx`:
```typescript
    ...
    <div className={cn("space-y-6", className)}>
      ...
      {data.grandOfficerStatus === 'Present' && (
        <SelectField
          ...
          options={roleOptions}
          required={required}
          error={errors.presentGrandOfficerRole}
        />
      )}
      ...
      <TextareaField
        ...
        rows={3}
        error={errors.grandOfficerDetails}
      />
    </div>
  );
};
...
2. Create a simplified version for display purposes:
```typescript
...
## Deliverables
- GrandOfficerFields component
- Conditional field display logic
- Status-based field rendering
- Summary display component
- Backward compatibility wrapper

## Success Criteria
- Only shows when rank is 'GL'
- Status field works correctly
- Role field shows for Present status
- Other input shows for Other role
