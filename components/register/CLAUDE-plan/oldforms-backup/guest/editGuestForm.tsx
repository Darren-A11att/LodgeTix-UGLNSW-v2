...
// import GuestPartnerToggle from './GuestPartnerToggle'; // Removed for edit form
...
const GuestForm: React.FC<GuestFormProps> = ({
  ...
  const registrationStore = useRegistrationStore((state: RegistrationState) => ({
    updateAttendee: state.updateAttendee,
    addGuestAttendee: state.addGuestAttendee,
    addGuestPartnerAttendee: state.addGuestPartnerAttendee,
    removeAttendee: state.removeAttendee,
    attendees: state.attendees,
    currentAttendee: state.attendees.find(att => att.attendeeId === attendeeId && att.attendeeType.toLowerCase() === 'guest'),
    partner: state.attendees.find(att => att.partnerOf === attendeeId && att.isPartner === true),
    primaryMasonAttendee: state.attendees.find(att => att.isPrimary === true && att.attendeeType.toLowerCase() === 'mason')
  }));
  
  const {
    updateAttendee: updateAttendeeInStore,
    addGuestAttendee: addGuestAttendeeStore,
    addGuestPartnerAttendee: addGuestPartnerAttendeeStore,
    removeAttendee: removeAttendeeInStore,
    attendees: allAttendeesFromStore,
    currentAttendee,
    partner,
  ...
  return (
    <div className="bg-slate-50 p-6 rounded-lg mb-8 relative">
      ...
      {/* --- Guest Partner Section Removed for Edit Form --- */}
      {/* Partner data is preserved but UI elements for toggling/displaying are removed */}
    </div>
  );
};
