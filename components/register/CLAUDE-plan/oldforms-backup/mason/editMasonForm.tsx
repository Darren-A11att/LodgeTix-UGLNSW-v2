...
import PartnerDetailsSection from '../guest/partner-details-section';
...
const MasonForm: React.FC<MasonFormProps> = ({
  ...
  const locationStore = useLocationStore((state: any) => ({
    grandLodges: state.grandLodges,
    isLoadingGrandLodges: state.isLoadingGrandLodges,
    grandLodgeError: state.grandLodgeError,
    allLodgeSearchResults: state.allLodgeSearchResults,
    isLoadingAllLodges: state.isLoadingAllLodges,
    allLodgesError: state.allLodgesError,
    fetchInitialGrandLodges: state.fetchInitialGrandLodges,
    searchGrandLodges: state.searchGrandLodges,
    searchAllLodgesAction: state.searchAllLodgesAction,
    createLodgeAction: state.createLodge
  }));
  ...
  return (
    <div className={`bg-slate-50 p-6 rounded-lg mb-8 relative`} id={`mason-form-${attendeeId}`}>
      ...
      {/* --- Lady Partner Section Removed for Edit Form --- */}
