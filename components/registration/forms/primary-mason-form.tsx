...
export function PrimaryMasonForm() {
  const setPrimaryAttendee = useRegistrationStore((state) => state.setPrimaryAttendee)
  const addAdditionalAttendee = useRegistrationStore((state) => state.addAdditionalAttendee)
  const currentPrimaryAttendee = useRegistrationStore((state) => state.attendeeDetails.primaryAttendee)
  ...
  const [formData, setFormData] = useState<Partial<MasonAttendee>>(() => {
    ...
    return currentPrimaryAttendee ? { ...currentPrimaryAttendee } : defaults
  })
  ...
  useEffect(() => {
    if (currentPrimaryAttendee) {
      setFormData({ ...currentPrimaryAttendee })
      setShowPartnerForm(!!currentPrimaryAttendee.hasPartner && !!currentPrimaryAttendee.partner)
    } else {
      setFormData({
        type: "mason",
        id: uuidv4(),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (
      ...
      showAlert(
        ...
        "Please fill in all required fields including name, lodge information, mobile number, and email address.",
        "warning"
      )
      return
    }

    const completeFormData = { ...formData, isPrimaryAttendee: true } as MasonAttendee
    setPrimaryAttendee(completeFormData)

    // If this form also handles submitting to a backend or next step, that logic would go here.
    // For now, it just updates the store.
    // Typically, a parent component (like a step) would handle navigation.
  }
  ...
  const handleTogglePartnerForm = () => {
    ...
    setFormData((currentFormData) => {
      const updatedFormData = { ...currentFormData, hasPartner: newShowPartnerForm };
      if (!newShowPartnerForm) {
        // If toggling off partner form, remove partner details from formData
        delete updatedFormData.partner;
      }
  ...
  const handlePartnerSubmit = (partnerData: PartnerAttendee) => {
    const updatedPrimaryMason = {
      ...formData,
      hasPartner: true,
      partner: {
        ...partnerData,
        id: formData.partner?.id || partnerData.id || uuidv4(),
      },
      isPrimaryAttendee: true,
    } as MasonAttendee
    
    setFormData(updatedPrimaryMason)
