...
import MasonForm from '../forms/mason/editMasonForm';
import UnifiedEditGuestForm from '../forms/guest/unified-edit-guest-form';
...
const AttendeeEditModal: React.FC<AttendeeEditModalProps> = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-2xl md:max-w-3xl lg:max-w-4xl p-0 h-modal-mobile md:h-auto max-h-modal-max">
        <Card className="shadow-none border border-masonic-lightgold rounded-modal">
          <CardHeader className="masonic-bg text-white rounded-t-modal p-reg-card-padding">
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-form-gap-sm">
          ...
          <CardFooter className="p-card-padding flex justify-end gap-form-gap bg-gray-50 border-t border-masonic-lightgold">
            <Button 
              variant="outline" 
              onClick={handleDiscardChanges}
              className="button-outline w-40"
            >
              Discard Changes
            </Button>
            <Button 
