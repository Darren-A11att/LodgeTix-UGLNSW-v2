-- Fix payment_status enum usage in upsert_individual_registration function
-- Ensure lowercase values are used consistently
...
BEGIN
    ...
    IF (p_registration_data->>'paymentCompleted')::boolean = true THEN
        ...
        UPDATE registrations SET
            payment_status = 'completed', -- lowercase enum value
    ...
    INSERT INTO registrations (
        ...
        v_payment_status::payment_status,  -- Explicitly cast to enum type
    ...
    -- Process tickets (FIXED: use correct column names)
    IF jsonb_array_length(p_registration_data->'tickets') > 0 THEN
        ...
        LOOP
            ...
            INSERT INTO tickets (
                ...
                payment_status,  -- Add payment_status with lowercase value
                created_at,
                updated_at
            ) VALUES (
                v_registration_id,
                ...
                'unpaid'::payment_status,  -- Use lowercase enum value with explicit cast
                CURRENT_TIMESTAMP,
                CURRENT_TIMESTAMP
            );
        END LOOP;
...
-- Add a debug function to check payment_status enum values
CREATE OR REPLACE FUNCTION public.debug_payment_status_enum() 
RETURNS TABLE (
    enum_label text
) AS $$
BEGIN
...
GRANT EXECUTE ON FUNCTION public.debug_payment_status_enum() TO authenticated;
GRANT EXECUTE ON FUNCTION public.debug_payment_status_enum() TO anon;