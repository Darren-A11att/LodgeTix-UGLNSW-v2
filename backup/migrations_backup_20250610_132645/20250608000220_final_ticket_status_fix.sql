-- Final comprehensive fix for ALL ticket_status enum casting errors
-- This migration ensures all RPC functions use varchar for ticket status

-- Drop and recreate all functions to ensure clean state
DROP FUNCTION IF EXISTS public.upsert_individual_registration(jsonb);
DROP FUNCTION IF EXISTS public.upsert_lodge_registration(uuid, uuid, integer, jsonb, jsonb, text, text, uuid, numeric, numeric, numeric, jsonb);
DROP FUNCTION IF EXISTS public.enhanced_lodge_registration(uuid, uuid, jsonb);
DROP FUNCTION IF EXISTS public.upsert_delegation_registration(jsonb);
DROP FUNCTION IF EXISTS public.upsert_masonic_registration(jsonb);

-- Recreate upsert_individual_registration without any ticket_status casting
...
BEGIN
    ...
    -- Process tickets with NO ticket_status casting
    IF p_registration_data->'tickets' IS NOT NULL THEN
        ...
        LOOP
            INSERT INTO tickets (
                ...
                'pending',  -- NO CASTING - just plain string
