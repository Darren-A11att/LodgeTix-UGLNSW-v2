-- Direct SQL to apply FK constraint removal to remote database
-- This removes the foreign key constraint and updates the RPC function

-- 1. FORCEFULLY DROP ALL RELATED CONSTRAINTS
DO $$
DECLARE
    constraint_record RECORD;
    constraint_sql TEXT;
BEGIN
    -- Find and drop ALL foreign key constraints on attendees table
    FOR constraint_record IN 
        ...
        AND constraint_name LIKE '%related_attendee_id%'
    LOOP
        constraint_sql := format('ALTER TABLE %I.%I DROP CONSTRAINT IF EXISTS %I', 
...
-- 2. UPDATED RPC FUNCTION WITHOUT COMPLEX RELATIONSHIP MAPPING
...
BEGIN
    ...
    -- Process primary attendee (NO RELATIONSHIP MAPPING)
    ...
    -- Process additional attendees (NO RELATIONSHIP MAPPING)
